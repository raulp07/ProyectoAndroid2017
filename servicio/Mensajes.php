<?php
	...
	class Mensaje{
		...
		public static function CrearTable($NameTable){
			try{
				$consultar  = "CREATE TABLE $NameTable (id VARCHAR(50) PRIMARY KEY,	mensaje VARCHAR(500) NOT NULL,tipo_mensaje VARCHAR(10) NOT NULL, hora_mensaje VARCHAR(50) NOT NULL)";
		...
		public static function EnviarMensaje($tableName,$id,$mensaje,$tipo_mensaje,$hora_mensaje){
			try {
				$consultar = "INSERT INTO $tableName (id,mensaje,tipo_mensaje,hora_mensaje) VALUES (?,?,?,?)";
				$respuesta = Database::getInstance()->getDb()->prepare($consultar);
				$respuesta->execute(array($id,$mensaje,$tipo_mensaje,$hora_mensaje));
				return 200;
			} catch (Exception $e) {
				return -1;
			}
